...
func (h *TicketHandler) ListTickets(c *gin.Context) {
    ...
    if email := c.Query("submitterEmail"); email != "" {
        query = query.Where("submitter_email = ?", email)
    }
...
// ExportTickets exports tickets in CSV or PDF format
func (h *TicketHandler) ExportTickets(c *gin.Context) {
    ...
    var tickets []models.Ticket
    if err := h.db.Order("created_at desc").Find(&tickets).Error; err != nil {
        c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to fetch tickets"})
        return
    }
    ...
    case "csv":
        ...
        // Write header
        header := []string{"Ticket Number", "Category", "Description", "Status", "Submitter", "Urgency", "Due Date", "Created At"}
        ...
        for _, t := range tickets {
            dueDate := ""
            if t.DueDate != nil {
                dueDate = t.DueDate.Format("2006-01-02 15:04:05")
            }
    ...
    case "pdf":
        ...
        // Header
        headers := []string{"Ticket #", "Category", "Status", "Submitter", "Urgency", "Due Date"}
        widths := []float64{30, 30, 25, 50, 25, 30}
        ...
        for _, t := range tickets {
            ...
            for i, cell := range cells {
                pdf.CellFormat(widths[i], 10, cell, "1", 0, "", false, 0, "")
            }
            pdf.Ln(-1)
        }
        ...
        c.Header("Content-Disposition", "attachment; filename=tickets.pdf")
