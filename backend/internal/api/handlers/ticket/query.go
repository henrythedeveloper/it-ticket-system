...
func (h *Handler) GetAllTickets(c echo.Context) error {
	...
	queryBuilder.WriteString(`
        ...
        LEFT JOIN users s ON t.end_user_email = s.email -- Join based on email to get submitter details
    `) // Changed t.submitter_id to t.end_user_email for join
	args := []interface{}{}
	paramCount := 0
	...
	if filters.FromDate != nil {
		paramCount++
		whereClauses = append(whereClauses, fmt.Sprintf("t.updated_at >= $%d", paramCount))
		args = append(args, *filters.FromDate)
	} // Filter by updated_at >= from_date
	if filters.ToDate != nil {
		paramCount++
		whereClauses = append(whereClauses, fmt.Sprintf("t.updated_at <= $%d", paramCount))
		args = append(args, (*filters.ToDate).Add(24*time.Hour-time.Nanosecond))
	} // Filter by updated_at <= to_date (end of day)
	...
	if filters.Search != "" {
		...
		whereClauses = append(whereClauses, fmt.Sprintf(`
            (t.subject ILIKE $%d OR t.description ILIKE $%d OR t.end_user_email ILIKE $%d OR CAST(t.ticket_number AS TEXT) ILIKE $%d)
...
func (h *Handler) GetTicketByID(c echo.Context) error {
	...
	err = h.db.Pool.QueryRow(ctx, `
        ...
        FROM tickets t
        LEFT JOIN users a ON t.assigned_to_user_id = a.id
        LEFT JOIN users s ON t.end_user_email = s.email -- Join based on email
        WHERE t.id = $1
    `, ticketID).Scan(
		&ticket.ID, &ticket.TicketNumber, &ticket.EndUserEmail, &ticket.IssueType, &ticket.Urgency,
		&ticket.Subject, &ticket.Description, &ticket.Status, &ticket.AssignedToUserID,
	...
	for i := 0; i < 3; i++ {
		...
		case attachments := <-attachmentsChan:
			for j := range attachments {
				attachments[j].URL = fmt.Sprintf("/api/attachments/download/%s", attachments[j].ID)
			}
	...
	if jsonErr != nil {
		logger.ErrorContext(ctx, "Failed to marshal final ticket data for logging", "error", jsonErr)
	} else {
		logger.DebugContext(ctx, "Final ticket data being sent in response", "ticketData", string(jsonData))
	}
...
func (h *Handler) SearchTickets(c echo.Context) error {
	...
	searchQuery := `
        ...
        WHERE ( t.subject ILIKE $1 OR t.description ILIKE $1 OR t.end_user_email ILIKE $1 OR CAST(t.ticket_number AS TEXT) ILIKE $1 )
    ` // Changed t.submitter_id to t.end_user_email
	searchPattern := "%" + query + "%"
...
func parseTicketStatus(statusStr string) *models.TicketStatus { /* ... same ... */
...
func parseTicketUrgency(urgencyStr string) *models.TicketUrgency { /* ... same ... */
...
func parseAssigneeFilter(assigneeStr, currentUserID string) *string { /* ... same ... */
	if assigneeStr == "me" {
		if currentUserID != "" {
...
func parseTags(tagsStr string) []string { /* ... same ... */
	if tagsStr == "" {
		return nil
	}
...
func parseInt(valueStr string, defaultValue int) int { /* ... same ... */
	if valueStr == "" {
		return defaultValue
	}
	var value int
...
func parseStringPtr(value string) *string { /* ... same ... */
	if value == "" {
		return nil
	}
	return &value
...
func parseSortBy(fieldStr, defaultField string, allowedFields []string) string {
	...
	for _, allowed := range allowedFields {
		if field == allowed {
			...
			switch field {
			case "createdat":
				return "created_at" // Example mapping
			case "updatedat":
				return "updated_at" // Example mapping
			default:
				return field // Assume direct match otherwise
			}
		}
	}
...
func (h *Handler) getTicketAttachments(ctx context.Context, ticketID string) ([]models.Attachment, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&attachment.ID, &attachment.TicketID, &attachment.Filename, &attachment.StoragePath, &attachment.MimeType, &attachment.Size, &attachment.UploadedAt); err != nil {
...
func (h *Handler) getTicketUpdates(ctx context.Context, ticketID string, requestingUserID string, requestingUserRole models.UserRole, ticketAssignedToUserID *string) ([]models.TicketUpdate, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&update.ID, &update.TicketID, &update.UserID, &update.Comment, &update.IsInternalNote, &update.CreatedAt, &authorID, &authorName, &authorEmail, &authorRole, &authorCreatedAt, &authorUpdatedAt); err != nil {
		...
		if update.UserID != nil && authorID != nil {
			author = models.User{ID: *authorID, Name: *authorName, Email: *authorEmail, Role: models.UserRole(*authorRole), CreatedAt: *authorCreatedAt, UpdatedAt: *authorUpdatedAt}
			update.User = &author
